...
def gen_main(designParamsVar):
    ...
    line += "  for(VAR_TYPE_32 paraLimbCounter=0; paraLimbCounter<PARA_LIMBS; paraLimbCounter++){" + "\n"
    line += "    bitReverseVect(tfArr[paraLimbCounter], size/2);" + "\n"
    line += "  }" + "\n"
    line += "" + "\n"
    ...
    line += "    tensorProd_NToR_fwd_NTT(fwdVec_ref[paraLimbCounter], tfArr[paraLimbCounter], workingModulus, size, NUM_BU);" + "\n"
    line += "  }" + "\n"
    line += "  printf(\"Host NTT computation completed\\n\");" + "\n"
    line += "" + "\n"
    ...
    if(designParamsVar.DOUBLE_BUF_EN):
        ...
        line += "  reorganize_input_poly_to_ports(invec, portWiseInPolyData);" + "\n"
        line += "" + "\n"
        ...
        line += "  std::vector<POLY_WIDE_DATA, tapa::aligned_allocator<POLY_WIDE_DATA>> portWiseInOutPolyData[POLY_LS_PORTS];" + "\n"
        line += "  reorganize_input_poly_to_ports(invec, portWiseInOutPolyData);" + "\n"
        line += "  init_kernerl_output_ports(portWiseInOutPolyData);" + "\n"
    line += "" + "\n"
    ...
    for port_idx in range(designParamsVar.POLY_LS_PORTS):
        if(designParamsVar.DOUBLE_BUF_EN):
            line += "      tapa::read_only_mmap<POLY_WIDE_DATA>(portWiseInPolyData[" + str(port_idx) + "])," + "\n"
        else:
            line += "      tapa::read_write_mmap<POLY_WIDE_DATA>(portWiseInOutPolyData[" + str(port_idx) + "])," + "\n"
    ...
    if(designParamsVar.DOUBLE_BUF_EN):
        for port_idx in range(designParamsVar.POLY_LS_PORTS):
            line += "      tapa::write_only_mmap<POLY_WIDE_DATA>(portWiseOutData[" + str(port_idx) + "])," + "\n"
    ...
    if(designParamsVar.DOUBLE_BUF_EN):
        line += "  reorganize_ports_to_output_poly(portWiseOutData, kernelOutArr);" + "\n"
    else:
        line += "  reorganize_ports_to_output_poly(portWiseInOutPolyData, kernelOutArr);" + "\n"
    line += "" + "\n"
    ...
    if(designParamsVar.DOUBLE_BUF_EN):
        ...
        line += "  reorganize_input_poly_to_ports(fwdVec_ref, inv_portWiseInPolyData);" + "\n"
        line += "" + "\n"
        ...
        line += "  std::vector<POLY_WIDE_DATA, tapa::aligned_allocator<POLY_WIDE_DATA>> inv_portWiseInOutPolyData[POLY_LS_PORTS];" + "\n"
        line += "  reorganize_input_poly_to_ports(fwdVec_ref, inv_portWiseInOutPolyData);" + "\n"
        line += "  init_kernerl_output_ports(inv_portWiseInOutPolyData);" + "\n"
    line += "" + "\n"
    ...
    for port_idx in range(designParamsVar.POLY_LS_PORTS):
        if(designParamsVar.DOUBLE_BUF_EN):
            line += "      tapa::read_only_mmap<POLY_WIDE_DATA>(inv_portWiseInPolyData[" + str(port_idx) + "])," + "\n"
        else:
            line += "      tapa::read_write_mmap<POLY_WIDE_DATA>(inv_portWiseInOutPolyData[" + str(port_idx) + "])," + "\n"
    ...
    if(designParamsVar.DOUBLE_BUF_EN):
        for port_idx in range(designParamsVar.POLY_LS_PORTS):
            line += "      tapa::write_only_mmap<POLY_WIDE_DATA>(inv_portWiseOutData[" + str(port_idx) + "])," + "\n"
    for para_limb_idx in range(designParamsVar.PARA_LIMBS):
        line += "      workingModulus_arr[" + str(para_limb_idx) + "]," + "\n"
    ...
    if(designParamsVar.DOUBLE_BUF_EN):
        line += "  reorganize_ports_to_output_poly(inv_portWiseOutData, inv_kernelOutArr);" + "\n"
    else:
        line += "  reorganize_ports_to_output_poly(inv_portWiseInOutPolyData, inv_kernelOutArr);" + "\n"
    line += "" + "\n"
